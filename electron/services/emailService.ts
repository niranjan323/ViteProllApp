 ...
 * Handles sending license requests via:
 * 1. Desktop email client (Mailto with attachment)
 * 2. SMTP (optional, requires nodemailer)
 */
 ...
 */
export function openEmailClientWithLicense(
  licenseRequestPath: string,
  ...
  return new Promise((resolve) => {
    try {
      ...
      shell.openExternal(mailtoLink).then(() => {
 ...
 */
export function openLicenseRequestFolder(folderPath: string): Promise<boolean> {
  return new Promise((resolve) => {
    try {
...
/**
 * SMTP Email Sending (requires nodemailer - optional)
 ...
 *   text: 'See attachment',
 *   attachments: [{ path: licenseRequestPath }]
 * });
 * ```
 */
 ...
 */
export function copyLicenseRequestToClipboard(filePath: string): Promise<boolean> {
  return new Promise((resolve) => {
    try {
 ...
 */
export function getEmailTemplate(licenseRequestPath: string): string {
